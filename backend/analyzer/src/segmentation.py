...
def is_chinese_word(token: str) -> bool:
    """Return True if token contains at least one Chinese character."""
    return bool(re.search(r'[\u4e00-\u9fff]', token))

def segment_with_freq(text: str) -> tuple[list[str], dict[str, int]]:
...
"""
    words, freq = segment_with_freq(sample)
    print("Words:", words)
